...
double* PowerMatrix(double* matrix, int size, int power) {
    ...
    for (int i=0; i<power-1; i++) {
        temp = Multiplication(temp, matrix, size, size, size, size);
    }
...
double* Inverse (double* matrix, int size) {
    ...
    for (int i=0; i<size; i++) {
        for (int j=0; j<size; j++) {
            minor = Minor(matrix, size, i+1, j+1);
            temp[i*size+j] = cofactor(minor, size-1, i+1, j+1);
        }
    }
    result = Transpose(temp, size, size);
    return result;
}
...
void Guass_Jordan_Elimination (double *Coffmatrix, double* Constmatrix, int equations, int variables) {
    ...
    for(int i=0; i<cols-1; i++) {
        if(free_variables[i]) {
            cout<<"x"<<i+1<<" = "<<"free variable"<<endl;
        }
        else {
            row = leadingPlacetoRow(reducedEchelonform, rank, cols, i+1)-1;
            cout<<"x"<<i+1<<" = "<<reducedEchelonform[row*cols+cols-1];      
            for(int j=i+1; j<cols-1; j++) {
                if(free_variables[j]) {
                    if(reducedEchelonform[row*cols + j] != 0) {
                        if(reducedEchelonform[row*cols + j] < 0)
                            cout<<" + "<<-reducedEchelonform[row*cols + j]<<"(x"<<j+1<<")";
                        else 
                            cout<<" - "<<reducedEchelonform[row*cols + j]<<"(x"<<j+1<<")";
                    }
                }
